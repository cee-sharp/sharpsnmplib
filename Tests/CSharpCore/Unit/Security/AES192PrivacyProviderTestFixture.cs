...
namespace CeeSharp.SnmpLib.Unit.Security
{
    ...
    public class AES192PrivacyProviderTestFixture
    {
        ...
        {
            var provider = new AES192PrivacyProvider(new OctetString("longlongago"),
                new MD5AuthenticationProvider(new OctetString("verylonglongago")));
            Assert.Throws<ArgumentNullException>(() => new AES192PrivacyProvider(null, null));
            Assert.Throws<ArgumentNullException>(() => new AES192PrivacyProvider(OctetString.Empty, null));
